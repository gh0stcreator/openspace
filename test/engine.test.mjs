...
test('круг: повторившемуся возвращают ход за другой мыслью', async () => {
  // Движок отвечает так, будто второй повторил первого своими словами: сперва тезисы
  // одним вызовом, дальше попарно — «одно» только для пары первый/второй.
  const think = async (p) => {
    if (/Вытащи из каждого ответа/.test(p)) {
      return ['первый | делать надо быстро',
        'второй | быстрота важнее всего',
        'третий | важна не скорость, а порядок'].join('\n');
    }
    return /не скорость, а порядок/.test(p) ? 'разное' : 'одно';
  };
...
test('внутренние мысли работают в разговорной комнате, а не мимо неё', async () => {
  ...
  const { orch } = setup(['первый', 'второй'], {
    think: async () => 'второй | задело | лень | 5 | быстро | вот это мимо',
...
test('своя мысль своей головой: спрашивают не всех и не общую модель', async () => {
  const общий = async () => [
    'первый | задело | лень | 4 | быстро | общая мысль первого',
    'второй | задело | лень | 4 | обдуманно | общая мысль второго',
    'третий | мимо | нечего | 2 | быстро | общая мысль третьего',
  ].join('\n');
  ...
  // Шаг «Поиск»: мысль формируется с памятью на руках, а не в пустоте. Задевает
  // участника как раз то, что расходится с тем, что он помнит.
  const { orch: помнящий } = setup(['первый', 'второй'], { think: общий, minds: () => '5 | своя мысль' });
  помнящий.memory = { brief: () => ({ text: 'решение: релиз не двигаем', told: false }) };
  const запомнил = [];
  помнящий.thinkAs = async (cfg, prompt) => { запомнил.push(prompt); return '5 | своя мысль'; };
  await помнящий.thoughts(ROOM, 'может, сдвинем релиз?');
  assert.ok(запомнил.every((p) => p.includes('релиз не двигаем')), 'память не доехала до мысли');

  // Память подбирается и по истолкованию реплики, а не только по её словам: запись,
  // нужная для ответа на «ну и?», по этим двум словам не найдётся никогда.
  const { orch: толкующий } = setup(['первый', 'второй'], { think: общий, minds: () => '5 | своя мысль' });
  const хвосты = [];
  толкующий.memory = { brief: (r, o) => { хвосты.push(o.tail); return { text: '', told: false }; } };
  толкующий.thinkAs = async () => '5 | своя мысль';
  await толкующий.thoughts(ROOM, 'ну и?');
  assert.ok(хвосты.some((x) => x.includes('общая мысль')), 'истолкование не попало в подбор памяти');
...
test('уверенность: путь по циклам и сумма, которой не может быть', async () => {
  ...
  // Поправка на уверенность: число нормируется, поэтому в одиночку завысить его нельзя —
  // «уверен на 95» имеет смысл только относительно чужих чисел.
  const { weighVotes } = await import('../lib/orchestrator.js');
  const rows = [{ name: 'А', votes: 3 }, { name: 'Б', votes: 2 }, { name: 'В', votes: 1 }];
  const ровно = weighVotes(rows, [['А', [90]], ['Б', [90]], ['В', [90]]]);
  assert.equal(ровно[0].name, 'А', 'при равных числах решают голоса');
  const сдвиг = weighVotes(rows, [['А', [20]], ['Б', [80]], ['В', [20]]]);
  assert.ok(сдвиг.find((r) => r.name === 'Б').share > ровно.find((r) => r.name === 'Б').share,
    'уверенность не подвинула счёт');
  assert.equal(весВсех(ровно), 100, 'доли не складываются в сотню');
  assert.deepEqual(weighVotes(rows, []), [], 'без чисел поправки не бывает');
  assert.deepEqual(weighVotes([], [['А', [50]]]), [], 'без голосов считать нечего');
});

/** Сумма долей: округление может дать 99 или 101, и это не ошибка счёта. */
function весВсех(rows) {
  const s = rows.reduce((a, r) => a + r.share, 0);
  return s >= 99 && s <= 101 ? 100 : s;
}

test('встречи не обрываются, пока кто-то ещё ни с кем не виделся', async () => {
  // Движок на всё отвечает «нет»: позиции считаются устоявшимися, и обрыв просится
  // после первого же цикла. При нечётном составе первому цикту достаётся одна пара,
  // а третий сидит на пустом стуле — и раньше уходил в защиту, не поговорив ни с кем.
  const { orch, calls } = setup(['первый', 'второй', 'третий'], { think: async () => 'нет' });
  orch.post('чёрная', { from: 'Roman', text: 'что делаем?' });
  await sleep(5000);

  const встречи = calls.filter((c) => c.step === 'встреча').map((c) => c.who);
  for (const кто of ['первый', 'второй', 'третий']) {
    assert.ok(встречи.includes(кто), `@${кто} ушёл в защиту, не встретившись ни с кем`);
  }
});

test('после круга отвечает тот, кого задело чужим ответом', async () => {
  // Круг кончился — четыре позиции лежат рядом. Дешёвая модель говорит, что задело
  // «второго»: он и берёт слово, не дожидаясь, пока человек ткнёт пальцем.
  const { orch, calls } = setup(['первый', 'второй', 'третий'], {
    think: async (p) => (/Повторы/.test(p)
      ? 'Повторы: нет'
      : 'второй | задело чужим ответом | нечего | 5 | обдуманно | так у нас не выйдет'),
    minds: () => '5 | так у нас не выйдет',
  });
  orch.post(CIRCLE, { from: 'Roman', text: 'как быть?' });
  await sleep(2500);

  const круг = calls.filter((c) => c.step === 'круг').map((c) => c.who);
  assert.equal(new Set(круг).size, 3, 'круг прошли не все');

  const после = calls.filter((c) => c.step === null).map((c) => c.who);
  assert.deepEqual(после, ['второй'], 'после круга не отозвался задетый — или отозвались лишние');
});

test('проверка круга сравнивает тезисы попарно', async () => {
  const { orch } = setup(['первый', 'второй', 'третий']);
  for (const кто of ['первый', 'второй', 'третий']) {
    orch.post(CIRCLE, { from: кто, text: `позиция ${кто}` });
  }
  const имена = ['первый', 'второй', 'третий'];

  // Тезисы вытаскиваются одним вызовом, дальше — по вызову на пару. Троих это три пары.
  const спросили = [];
  orch.think = async (p) => {
    спросили.push(p);
    if (/Вытащи из каждого ответа/.test(p)) {
      return '**первый** | дистрибуция первым фильтром\nвторой | дистрибуция первым фильтром\nтретий | сначала считать деньги';
    }
    // Одинаковыми считаем только первую пару: первый и второй.
    return /сначала считать деньги/.test(p) ? 'разное' : 'одно';
  };
  const повторы = await orch.echo(CIRCLE, имена);

  assert.equal(спросили.length, 4, 'ожидали один вызов на тезисы и три на пары');
  assert.equal(повторы.length, 1, 'повтор не найден или найден лишний');
  assert.equal(повторы[0][0], 'второй', 'ход возвращают не тому: повторился второй');

  // Пустой ответ на тезисы круг не роняет и повторов не выдумывает.
  orch.think = async () => '';
  assert.deepEqual(await orch.echo(CIRCLE, имена), []);

  // Все разные — никого не возвращаем.
  orch.think = async (p) => (/Вытащи/.test(p)
    ? 'первый | раз\nвторой | два\nтретий | три'
    : 'разное');
  assert.deepEqual(await orch.echo(CIRCLE, имена), []);
});
...
test('сходимость проверяется одним вызовом на комнату', async () => {
  const { orch } = setup(['первый', 'второй', 'третий']);
  const спросили = [];
  orch.think = async (p) => { спросили.push(p); return 'первый | нет\nвторой | нет\nтретий | да'; };
  for (const кто of ['первый', 'второй', 'третий']) {
    orch.post('чёрная', { from: кто, text: 'позиция раз' });
    orch.post('чёрная', { from: кто, text: 'позиция два' });
  }
  const сошлись = await orch.settled('чёрная', ['первый', 'второй', 'третий']);
  assert.equal(спросили.length, 1, 'вызовов больше одного');
  assert.ok(сошлись, 'сдвинулся один из трёх — это шум, а не работа цикла');
  assert.match(спросили[0], /@первый[\s\S]*@второй[\s\S]*@третий/, 'в вызов попали не все');

  // Тот, кому не с чем сравнивать, больше не отвечает «не сошлись» за всех.
  const { orch: новый } = setup(['первый', 'второй']);
  новый.think = async () => 'первый | нет\nвторой | нет';
  новый.post('чёрная', { from: 'первый', text: 'раз' });
  новый.post('чёрная', { from: 'первый', text: 'два' });
  новый.post('чёрная', { from: 'второй', text: 'только одна' });
  assert.equal(await новый.settled('чёрная', ['первый', 'второй']), false,
    'сравнивать было не с чем — досрочной остановки быть не должно');
});

test('протокол встаёт по потолку стоимости и говорит цену', async () => {
  // Потолок в один токен: первый же ход его перебирает, и протокол обязан
  // остановиться сам, а не идти полсотни ходов.
  const { orch } = setup(['первый', 'второй', 'третий'], {
    think: async () => 'нет',
    weight: { первый: 5000, второй: 5000, третий: 5000 },
  });
  orch.config.protocolTokens = 1;
  orch.post('чёрная', { from: 'Roman', text: 'что делаем?' });
  await sleep(3000);

  const весть = orch.store.load('чёрная').find((m) => m.kind === 'system' && /потолок/.test(m.text));
  assert.ok(весть, 'о потолке стоимости не сказали');
  assert.match(весть.text, /тысяч токенов/);
  assert.equal(orch.state('чёрная').stopped, 'встречи', 'фаза обрыва не запомнена');
});

test('один файл на двоих после круга виден человеку', async () => {
  const { orch } = setup(['первый', 'второй']);
  orch.post(CIRCLE, { from: 'первый', text: 'вот', files: [{ name: 'тз.md', size: 1, url: '/u', path: 'drafts/тз.md' }] });
  orch.post(CIRCLE, { from: 'второй', text: 'и вот', files: [{ name: 'тз.md', size: 1, url: '/u', path: 'drafts/тз.md' }] });
  orch.collide(CIRCLE, 0);
  const весть = orch.store.load(CIRCLE).find((m) => m.kind === 'system' && /Один файл на двоих/.test(m.text));
  assert.ok(весть, 'о затёртой работе не сказали');
  assert.match(весть.text, /@первый и @второй|@второй и @первый/);
  assert.match(весть.text, /drafts\/тз\.md/);
});

test('круг, помеченный идущим при пустой очереди, не делает комнату глухой', async () => {
  // Так остаётся после обрыва на перезапуске: метка есть, ждать некого — и раньше
  // любая реплика молча отбрасывалась в самом начале dispatch.
  const { orch, calls } = setup(['первый', 'второй'], { think: async () => '' });
  const st = orch.state(ROOM);
  st.blind = true;
  st.pending = [];
  orch.post(ROOM, { from: 'Roman', text: 'эй, @первый' });
  await sleep(400);
  assert.equal(orch.state(ROOM).blind, false, 'метка круга не снята');
  assert.ok(calls.some((c) => c.who === 'первый'), 'комната осталась глухой');
});

test('оборванный протокол говорит, где встал, и доигрывается с защиты', async () => {
  const { orch, calls } = setup(['первый', 'второй', 'третий'], { think: async () => 'нет' });
  orch.post('чёрная', { from: 'Roman', text: 'что делаем?' });
  // Пауза посреди протокола: так он и умирал молча. Ловим его на первых же ходах —
  // на заглушках весь протокол пробегает за секунду.
  await sleep(150);
  orch.state('чёрная').paused = true;
  await sleep(3000);

  const отметка = orch.store.load('чёрная').find((m) => m.kind === 'system' && /остановлен на фазе/.test(m.text));
  assert.ok(отметка, 'обрыв прошёл молча');
  assert.match(отметка.text, /доиграет с защиты/);
  assert.equal(orch.state('чёрная').stopped, 'встречи', 'фаза обрыва не запомнена');

  // Реплика человека доигрывает, а не заводит новый круг.
  const былоКругов = calls.filter((c) => c.step === 'круг').length;
  orch.post('чёрная', { from: 'Roman', text: 'продолжайте' });
  await sleep(4000);
  assert.equal(calls.filter((c) => c.step === 'круг').length, былоКругов, 'вместо защиты пошёл новый круг');
  assert.ok(calls.some((c) => c.step === 'защита'), 'защита не началась');
});

test('проверку выполнимости делает тот, кто меньше всех вложен в победителя', async () => {
  const { orch, calls } = setup(['первый', 'второй', 'третий'], {
    think: async () => 'нет',
    // Голосуют все за «первого»: он победитель, и проверять его решение должен не он.
    reply: 'за: первый\nпрогноз: первый\nпочему: так',
  });
  orch.post('чёрная', { from: 'Roman', text: 'что делаем?' });
  await sleep(7000);

  const проверка = calls.filter((c) => c.step === 'проверка');
  assert.equal(проверка.length, 1, 'проверка не прошла или прошла дважды');
  assert.notEqual(проверка[0].who, 'первый', 'победитель проверяет сам себя');

  const отметка = orch.store.load('чёрная').find((m) => m.kind === 'system' && /Проверка:/.test(m.text));
  assert.ok(отметка, 'человеку не сказали, что идёт проверка');
  assert.match(отметка.text, /@первый/, 'не названо, чьё решение проверяют');
});
