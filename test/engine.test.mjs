...
test('на реплику отвечает тот, чья это тема, а не дежурный', async () => {
  const { orch, calls } = setup(['первый', 'второй'], { roles: { первый: 'дизайнер' } });

  // Дежурный в тестовом «Открытом» — @второй. Но тема дизайнерская, и отвечает Дизайнер:
  // разговор про вёрстку, в котором отвечает не он, — это не разговор про вёрстку.
  ...
  assert.ok(!calls.some((c) => c.who === 'второй'), 'дежурный влез в чужую тему');

  // Чужая тема его не трогает: слово из `off` держит вернее, чем совпадение из `on`.
  // Раз тема не зацепила никого — отвечает дежурный.
  calls.length = 0;
  ...
  assert.ok(calls.some((c) => c.who === 'второй'), 'без темы должен отвечать дежурный');
});
