...
test('проверка круга разбирает повтор в любой обёртке', async () => {
  const { orch } = setup(['первый', 'второй', 'третий']);
  for (const кто of ['первый', 'второй', 'третий']) {
  ...
  // Ответ моделью в разметке: раньше такой выбрасывался целиком, и проверка молчала
  // не потому, что повторов нет, а потому что они были написаны жирным.
  orch.think = async () => '**первый** | тезис\n**второй** | тезис\n\n**Повторы:** второй — первый';
  const вжирном = await orch.echo(CIRCLE, имена);
  assert.equal(вжирном.length, 1, 'повтор в разметке не разобран');
  assert.equal(вжирном[0][0], 'второй');

  // Обычный ответ без разметки разбирается так же.
  orch.think = async () => 'первый | тезис\nвторой | тезис\nПовторы: второй — первый';
  assert.equal((await orch.echo(CIRCLE, имена))[0][0], 'второй');

  // «Повторы: нет» — это ноль, а не ошибка разбора.
  orch.think = async () => 'первый | тезис\nПовторы: нет';
  assert.deepEqual(await orch.echo(CIRCLE, имена), []);

  // Пустой ответ движка не выдаёт повторов и не роняет круг.
  orch.think = async () => '';
  assert.deepEqual(await orch.echo(CIRCLE, имена), []);
});
