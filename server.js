...
const short = (m) => {
  ...
  return {
    ...
    talk: !!m.talk,
    // Уклад комнаты: её законы своими словами. Зашедшему в пустую комнату он и объясняет,
    // что здесь происходит, — иначе объяснять это пришлось бы вторым текстом, который
    // разойдётся с первым в первый же день.
    laws: m.laws ?? '',
    // Как здесь идёт разговор. Одним словом, потому что человеку нужно поведение,
    // а не устройство: «встречи» важнее, чем то, какие поля за ними стоят.
    flow: m.meet ? 'встречи' : (m.talk ? 'разговор' : (m.circle ? 'круг' : 'очередь')),
  };
};
