...
export class Store {
  ...
  listRooms() {
    const onDisk = fs.existsSync(this.dir)
      // `.rounds.jsonl` — журнал счётчика вопросов, который больше не пишется, но на машинах
      // остался: без этого фильтра он показывался комнатой и поднимался при старте.
      ? fs.readdirSync(this.dir).filter((f) => f.endsWith('.jsonl') && !f.endsWith('.rounds.jsonl')
