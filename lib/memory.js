...
export class Memory {
  ...
  brief(room, limit = 8000) {
    ...
    // Уроки идут сразу за ограничениями: они тоже запрещают и разрешают, только
    // не в предмете, а в том, как работать, — и если память обрежется, терять их последними.
