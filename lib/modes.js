...
/**
 * Шаги из текста. Режим правят одним полем — так же, как голос участника, — поэтому
 * разбор текста и сборка текста живут рядом и обязаны быть обратны друг другу.
 */
...
/** Шаги в текст — то, что человек видит и правит в поле. */
export function bodyOf(steps) {
  return (steps ?? []).map((st) => [
    `## ${st.name}`,
    `who: ${st.who || 'все'}`,
...
/**
 * Стороны режима: кто в нём за что. Роль отвечает на «что участник делает вообще»,
 * сторона — на «кто он в этом разговоре»: в дебатах Скептик спорит не как скептик,
 * а как «против», и по ленте это должно быть видно без чтения реплик.
 * Строкой: «За/thumbs-up: креатор, инженер | Против/thumbs-down: скептик, академик».
 */
function sidesOf(meta) {
  ...
  return String(meta.sides ?? '').split('|').map((chunk, i) => {
    ...
    return {
      ...
      labelEn: en[i] ?? '',
      icon: (m[2] ?? '').trim(),
      // Цвет необязателен: у стороны спора его нет — цвет там опознаёт участника.
...
/** Стороны обратно в строку файла: разбор и сборка обязаны быть обратны друг другу. */
...
/**
 * Режим, которому принадлежит этот ник. Персонаж, которого не с кем спутать в обычном
 * составе — второй дизайнер по имени Бараш, — живёт не в команде, а в своём режиме:
 * вне его он не отвечает и в списках не показывается. Узнаём по стороне с таким именем.
 */
/**
 * Настрой места: насколько охотно здесь вступают в разговор. Пороги — из профилей
 * Inner Thoughts (CHI 2025), где их калибровали на живых людях, и они не одинаковы
 * для всякой работы: болтовня, разгон идей, рабочая встреча и разбор чужого решения
 * требуют разной готовности открыть рот.
 *
 * `говорит` — балл, с которого мысль просится вслух; `перебивает` — с которого можно
 * влезть в реплику, адресованную другому; `искра` — доля случаев, когда при отсутствии
 * сильных мыслей всё равно кто-то отзовётся; `смотрит` — на что обращать внимание
 * при оценке своей же мысли.
 */
export const TUNES = {
  разговор: {
    говорит: 3.6, перебивает: 4.8, искра: 0.2,
    смотрит: 'зацепило ли это лично его и есть ли что сказать своё',
  },
  разгон: {
    говорит: 3.8, перебивает: 4.9, искра: 0.1,
    смотрит: 'неожиданность мысли и то, насколько она сдвинет разговор; повтор уже сказанного — балл вниз',
  },
  встреча: {
    говорит: 4.0, перебивает: 4.2, искра: 0,
    смотрит: 'пробел в сказанном, несостыковку и уход от темы; просто согласие — балл вниз',
  },
  спор: {
    говорит: 3.5, перебивает: 4.5, искра: 0,
    смотрит: 'противоречие, риск и слабое место в доводе; вежливое согласие — балл вниз',
  },
};

/** Настрой режима: у разговора свой, у работы — рабочий. */
export function tuneOf(mode) {
  return TUNES[mode?.tune] ?? (mode?.talk ? TUNES.разговор : TUNES.встреча);
}

export function modeOfName(nick) {
  const who = String(nick ?? '').trim().toLowerCase();
  if (!who) return '';
  return listModes().find((m) => (m.sides ?? []).some((x) => x.label.toLowerCase() === who))?.name ?? '';
}

/** Сторона участника — по его роли. Нет сторон у режима — нет и стороны. */
export function sideOf(mode, role) {
  const r = String(role ?? '').toLowerCase();
  return (mode?.sides ?? []).find((s) => s.roles.includes(r)) ?? null;
}
...
function parse(name, raw) {
  ...
  if (m) {
    for (const line of m[1].split('\n')) {
      // Ключ может быть кириллицей: «настрой» — такое же поле шапки, как slug или icon.
      const kv = line.match(/^([\wЀ-ӿ]+):\s*(.*)$/);
      if (kv) meta[kv[1]] = kv[2].trim();
    }
  }

  const steps = stepsFrom(body);
  ...
  return {
    ...
    // Кто в режиме за что: пары ролей с подписью и знаком.
    sides: sidesOf(meta),
    // Режим без регламента: ходы идут как в открытом разговоре, шагов нет. Нужен там,
    // где режим меняет не порядок ходов, а то, кем участники в нём выходят.
    talk: /^(да|yes|true)$/i.test(String(meta.talk ?? '')),
    // Настрой: разговор, разгон, встреча или спор. Пусто — по типу режима.
    tune: TUNES[String(meta['настрой'] ?? '').trim()] ? String(meta['настрой']).trim() : '',
...
export function listModes() {
  ...
  return fs.readdirSync(DIR)
    ...
    // Встроенный первым, режимы без регламента — последними. Они не про порядок работы,
    // а про то, кем в них выходят, и в одном ряду со «Штабом» читаются как ещё один приём.
    .sort((a, b) => {
...
export function saveMode(mode) {
  ...
  const head = [
    ...
    `talk: ${mode.talk ? 'да' : 'нет'}`,
    `настрой: ${mode.tune ?? ''}`,
    `sides: ${sidesLine(mode.sides)}`,
    `sides_en: ${(mode.sides ?? []).map((s) => s.labelEn ?? '').join(' | ')}`,
    '---',
    '',
  ];
