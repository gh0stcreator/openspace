...
function parse(name, raw) {
  ...
  return {
    ...
    // С кем роль хорошо сходится и на чём. Не пара по оси — та про спор, а эта про работу:
    // «роль — и с кем она лучше всего сочетается» стояло отдельной графой ещё в таблице,
    // по которой собирали смешариков, и это единственное, чего в нашей не было.
    // Формат: «инженер — идея превращается в вещь».
    goes: (meta['сочетается'] ?? '').trim(),
