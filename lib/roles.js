 ...
 * в разные стороны: роль, которая ни с кем не спорит, — лишний рот. Главная черта объявлена
 * в самой роли полем `черта`, пара считается отсюда.
 *
 * Продюсер и Архивариус черты не имеют, и это не пропуск: первый ведёт разговор,
...
/** Противоположная черта на той же оси. Неизвестная черта — пусто, а не догадка. */
...
function parse(name, raw) {
  ...
  return {
    ...
    // Главная черта роли — осторожность, теория, вкус. Она же ставит роль на конец оси,
    // и пару код считает сам: спорит тот, у кого черта противоположная. Записываем черту,
    // а не имя оппонента, нарочно: черта говорит, что делать, и работает в любом составе,
    // а имя требует, чтобы этот участник существовал, и без него роль остаётся без позиции.
    // Старые имена поля (`полюс`, `тянет`) читаются по-прежнему: файлы ролей правят руками.
    pulls: (meta['черта'] ?? meta['полюс'] ?? meta['тянет'] ?? meta.pulls ?? '').trim().toLowerCase(),
    // Напарник: с кем этой роли вместе получается вещь и на чём именно. Не пара по оси —
    // та про спор, эта про работу. Графа «с кем лучше всего сочетается» стояла ещё
    // в таблице, по которой собирали смешариков, и это единственное, чего у нас не было.
    // Формат: «инженер — идея превращается в вещь».
    partner: (meta['напарник'] ?? meta['сочетается'] ?? '').trim(),
