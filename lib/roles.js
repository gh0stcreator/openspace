 ...
 * в самой роли полем `полюс`, пара считается отсюда.
 *
...
function parse(name, raw) {
  ...
  return {
    ...
    // На каком полюсе оси стоит роль: с противоположным она и спорит. Пусто — роль вне пар.
    // Старое имя поля `тянет` читается по-прежнему: файлы ролей правят руками, и ломать
    // их переименованием ради красоты — плохая сделка.
