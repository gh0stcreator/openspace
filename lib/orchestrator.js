...
const MEMORY_KINDS = new Set(['решение', 'ограничение', 'находка', 'допущение', 'вопрос']);
...
export class Orchestrator {
  ...
  flush(room) {
    ...
    try {
      writeState(this.stateDir, room, {
        ...
        off: st.off, archiveUpto: st.archiveUpto, seen, sessions,
      });
    } catch (e) {
      this.log.error(e);
    }
  ...
  state(room) {
    if (!this.rooms.has(room)) {
      ...
      this.rooms.set(room, {
        ...
        off: kept?.off ?? [],
        // Курсор свёртки памяти: свой, не agent.lastSeen — свёртку делает не выделенный
        // архивариус, а любой нанятый на один ход, и его личный курсор чтения это не он.
  ...
  confirmMemory(room, seq) {
    ...
    return this.store.resolveMemory(room, seq, 'принято');
  }
  ...
  dispatch(room, msg) {
    ...
    if (!targets.length) {
      if (msg.from === this.config.user) {
        ...
        // Плюс один, кого тема задела сильнее прочих: разговор про вёрстку без Дизайнера —
  ...
  async speakTurn(room, name, agent, ctx) {
    ...
    // Свёртка — шапка на один ход, не смена личности: курсор чтения участника (agent.lastSeen)
    // её не касается, у неё свой — ctx.since, курсор свёртки комнаты, не связанный с тем,
    // что этот же участник успел прочитать как собеседник.
    ...
    // на diff нечем, это дело человека и клика, а не разговора. Свои же реплики обычно не
    // показываем — эхо самому себе бессмысленно, но на свёртке «сам себе» и значит «пропустить
    // то, что сказал участник, которому в этот раз досталась шапка» — свёртке нужны все.
    let delta = this.store.since(room, seen ?? 0)
      .filter((m) => (ctx.archive || m.from !== name) && !['system', 'memory-proposal', 'memory-resolved'].includes(m.kind));
    ...
    try {
      ...
      // Свёртка памяти — не реплика в разговоре: того, кто на этот ход надел шапку,
      // читает код, не собеседники. Его ответ не идёт в ленту текстом, а разбирается
      // в diff и ложится предложением; личный курсор чтения участника не двигаем —
      // свёртка не то, что он «прочитал» как собеседник, у неё свой курсор комнаты.
      if (ctx.archive) {
        ...
        this.save(room);
        if (diff.length) {
          this.store.append(room, { from: name, kind: 'memory-proposal', diff, status: 'ожидает', mentions: [] });
        }
        return true;
      }
  ...
  /**
   * Прогнать свёртку памяти от её курсора до конца ленты, порциями по 50. Свёртка — не
   * отдельный найм, а шапка на один ход: делает её любой уже нанятый, а курсор — свой,
   * st.archiveUpto, не agent.lastSeen. Отдельный курсор не роскошь: lastSeen — это то,
   * что участник прочитал как собеседник, а свёртку он не читает как собеседник, её
   * ему поручают; смешать эти две вещи значит один раз в архиве сдвинуть его настоящий
   * курсор и заставить пропустить кусок разговора, который он «как бы уже видел».
   */
  async archive(room) {
