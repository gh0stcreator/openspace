...
import { ask, buildAgent } from './agents.js';
import { dropState, readState, writeState } from './state.js';
import { parseMentions, resolveTargets } from './mentions.js';
import { roleOf } from './roles.js';
import { BUILTIN, listModes, loadMode, modeOfName, sideOf, stepTargets, tuneOf } from './modes.js';
...
const chatty = (st) => !!(st.mode && loadMode(st.mode).talk);
...
export class Orchestrator {
  constructor({ store, config, log = console, build = buildAgent, stateDir = null, think = ask }) {
    this.store = store;
    this.config = config;
    this.log = log;
  ...
  async thoughts(room, text, exclude = []) {
    ...
    const card = (n) => {
      ...
      return `- @${face(n)}: ${cares}.${motive}${mind}`;
    };
    const answer = await this.think(`В разговоре прозвучало:\n«${String(text).slice(0, 1200)}»\n\n`
      + `Вот кто это слышал:\n${pool.map(card).join('\n')}\n\n`
      + `Здесь смотрят на ${tuneOf(loadMode(this.state(room).mode)).criteria}.\n\n`
      + `На каждого — одна строка вида «Ник | балл | мысль». Мысль от первого лица, до ста `
      + `знаков, та самая, что мелькнула бы в голове. Балл ставь, взвесив обе стороны: `
      + `назови себе одну причину сказать это вслух сейчас и одну причину промолчать, `
      + `и только потом выбирай число — иначе все баллы выходят завышенными.\n`
      + `5 — молчать невозможно, это прямо про него или прямо неверно;\n`
      + `4 — есть что добавить по делу;\n`
      + `3 — мысль есть, но и без неё обойдутся;\n`
      + `2 — так, отметил про себя;\n`
      + `1 — мимо, сказать нечего.\n\n`
      + `Большинству ставь 1 или 2: в живом разговоре на каждую реплику отзываются один-двое, `
      + `а не все. Пятёрка — редкость. У кого в голове уже лежит мысль — не повторяй её `
      + `слово в слово: доверни с учётом услышанного или замени, если разговор ушёл. `
      + `Ничего, кроме строк, не пиши.`,
    { cwd: this.config.workdir });
    if (!answer) return this.drawn(room, text, exclude, 1).map((name) => ({ name, score: 4, thought: '' }));
    const out = [];
    for (const line of answer.split('\n')) {
      const m = line.match(/^\s*@?([^|]+?)\s*\|\s*([1-5])\s*\|\s*(.*)$/);
      if (!m) continue;
      const who = pool.find((n) => face(n).toLowerCase() === m[1].trim().toLowerCase());
      if (who && !out.some((x) => x.name === who)) {
        out.push({ name: who, score: Number(m[2]), thought: m[3].trim() });
      }
    }
  ...
  speaks(room, mind, floor) {
    const tune = tuneOf(loadMode(this.state(room).mode));
  ...
  runStep(room, run) {
    this.enqueue(room, async () => {
      ...
      if (step.hear) for (const name of crowd) await ask(name);
      else await Promise.all(crowd.map(ask));

      if (!live() || st.pending.length) return;

      // Слепой круг мог схлопнуться: писали не глядя друг на друга, а вышло одно и то же
      // разными словами. Дальше по такому кругу спорить не о чем — обсуждение начнётся
      // с одной позиции, размноженной на всех. Тем, кто повторился, возвращаем ход.
      if (!step.hear && crowd.length > 2) {
        ...
        for (const [name, same] of again) {
          const say = `\n\nТвою мысль уже сказал другой: «${same}». Она не твоя и не новая. `
            ...
            + 'его: на этом шаге ценно расхождение, а не согласие.';
          st.pending = [name];
          await ask(name, say);
        }
        if (!live() || st.pending.length) return;
  ...
  async dispatch(room, msg) {
    ...
    if (!targets.length) {
      if (msg.from === this.config.user) {
        ...
        const CHATTY = 3;
        if (!targets.length && chatty(st) && st.autoTurns < CHATTY) {
    ...
    if (!targets.length) {
      ...
      const free = (!st.mode || loadMode(st.mode).talk) && this.config.freeTalk !== false;
    ...
    if (chatty(st) && targets.length) {
      ...
      this.thoughts(room, msg.text, skip)
        .then((mind) => {
          ...
          const [top] = this.speaks(room, mind, tuneOf(loadMode(st.mode)).intervene);
