...
const SAY_AT = 4;      // imThreshold 3.59 у них — у нас балл целый, ближайший порог четыре
const HOLD_AT = 3;     // мысль есть, но и без неё обойдутся: в голову, не вслух
const BUTT_IN = 5;     // interruptThreshold 4.8: влезть в чужой разговор — только с этим
const SPARK = 0.2;     // system1Prob 0.2: быстрая реакция там, где сильных мыслей нет
...
export class Orchestrator {
  ...
  async thoughts(room, text, exclude = []) {
    ...
    const answer = await ask(`В разговоре прозвучало:\n«${String(text).slice(0, 1200)}»\n\n`
      + `Вот кто это слышал:\n${pool.map(card).join('\n')}\n\n`
   ...
   */
  speaks(room, mind, floor = SAY_AT) {
    const loud = mind.filter((m) => m.score >= floor).slice(0, 2);
    ...
    const spark = rest[0];
    if (spark && floor === SAY_AT && Math.random() < SPARK) return [spark.name];
    return [];
  }
    ...
    if (chatty(st) && targets.length) {
      ...
      this.thoughts(room, msg.text, skip)
        .then((mind) => {
          ...
          const [top] = this.speaks(room, mind, BUTT_IN);
