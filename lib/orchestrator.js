...
import { listSpaces, loadSpace, pick, sideOf, tuneOf } from './spaces.js';
...
export class Orchestrator {
  ...
  flush(room) {
    ...
    try {
      writeState(this.stateDir, room, {
        blind: st.blind, circleAt: st.circleAt, run: st.run, pending: st.pending,
  ...
  circle(room) {
    ...
    if (!space.circle || cast.length < 3) return false;
  ...
  async dispatch(room, msg) {
    ...
    if (!targets.length) {
      if (msg.from === this.config.user) {
        ...
        // работает. Подряд два круга не заводим: пока на прошлый никто не ответил,
        // новая реплика человека — уточнение, а не новый вопрос.
        const said = this.store.load(room)
          .some((m) => m.kind === 'message' && m.from !== this.config.user && m.seq > (st.circleAt ?? 0));
        if ((!st.circleAt || said) && this.circle(room)) return;
