...
export class Orchestrator {
  ...
  state(room) {
    if (!this.rooms.has(room)) {
      this.rooms.set(room, { autoTurns: 0, paused: false, mode: null, step: 0, jobs: [], running: false });
    }
  ...
  modeState(room) {
    ...
    return {
      ...
      waitingUser: step?.until === 'человек',
    };
  }
  ...
  nextStep(room) {
    ...
    if (!step) {
      ...
      this.store.append(room, {
        from: 'system',
        kind: 'system',
        text: `@${this.config.user}, круги пройдены — слово за тобой`,
  ...
  dispatch(room, msg) {
    ...
    if (st.mode) {
      ...
      if (msg.from === this.config.user && step?.until === 'человек') this.nextStep(room);
      return;
    }
  ...
  duty() {
    const step = loadMode('свободный').steps[0];
