...
export class Orchestrator {
   ...
   * при этом не меняется, иначе от смены имени ломались бы режимы, пары и домены.
   */
  personas(room) {
    const st = this.state(room);
    ...
    for (const name of this.here(room)) {
      ...
      const side = sides.find((x) => x.roles.includes(role));
      if (!side) continue;
      // Голос берём из файла амплуа с тем же именем. Нет файла — это не персона,
      // а сторона: «Против» — место в споре, а не другой человек.
