  ...
  dispatch(room, msg) {
    ...
    if (!targets.length) {
      if (msg.from === this.config.user) {
        ...
        const chat = !!(st.mode && loadMode(st.mode).talk);
        ...
        // шести ходов подряд без человека разговор идёт уже без него.
        const CHATTY = 6;
        if (!targets.length && st.mode && loadMode(st.mode).talk && st.autoTurns < CHATTY) {
          const rest = this.talkers(room).filter((n) => n !== msg.from && !waiting.includes(n));
          targets = this.quietest(room, rest);
        }
      }
    }
