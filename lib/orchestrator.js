...
export class Orchestrator {
  ...
  async thoughts(room, text, exclude = []) {
    ...
    const answer = await ask(`В разговоре прозвучало:\n«${String(text).slice(0, 1200)}»\n\n`
      + `Вот кто это слышал:\n${pool.map(card).join('\n')}\n\n`
      + `Здесь смотрят на ${tuneOf(loadMode(this.state(room).mode)).смотрит}.\n\n`
  ...
  speaks(room, mind, floor) {
    ...
    const spark = rest[0];
    if (spark && floor === undefined && Math.random() < tune.искра) return [spark.name];
    return [];
  }
    ...
    if (chatty(st) && targets.length) {
      ...
      this.thoughts(room, msg.text, skip)
        .then((mind) => {
          ...
          const [top] = this.speaks(room, mind, tuneOf(loadMode(st.mode)).перебивает);
