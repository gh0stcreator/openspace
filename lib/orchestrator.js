...
export class Orchestrator {
  ...
  flush(room) {
    ...
    try {
      writeState(this.stateDir, room, {
        ...
        topic: st.topic, topicAt: st.topicAt, seen, sessions,
      });
    } catch (e) {
      this.log.error(e);
    }
  ...
  view(room) {
    ...
    return { autoTurns, paused, busy, modeState: this.modeState(room) };
  }
   ...
   * теневой найм — участник с ролью «архивариус», исключённый из here()/duty()/resonates():
  ...
  /**
   * Одно слово о том, чем комната занята сейчас. Спрашиваем не «какая тема», а «та же или
  ...
  async retopic(room) {
    ...
    st.topicAt = this.store.load(room).at(-1)?.seq ?? 0;
    try {
      const probe = this.build('claude', {
        kind: 'claude', model: 'haiku', lean: true, trust: 'safe', timeoutMs: 60000,
      });
      const now = st.topic || '—';
      const res = await probe.speak({
        ...
        step: {
          name: 'тема',
          hear: true,
          prompt: 'На этот ход ты не участник разговора и в чат не отвечаешь. Нужно одно слово.\n\n'
            + `Разговор помечен темой «${now}». Прочитай реплики и реши: они про то же самое `
            + 'или разговор ушёл в другое.\n\n'
            + 'Слово называет ПРЕДМЕТ разговора, а не занятие. О чём говорят, а не что с этим '
            + 'делают: article, pricing, onboarding, memory — но не validation, не review, '
            + 'не discussion. Если всё крутится вокруг одной вещи, которую принёс человек, — '
            + 'назови эту вещь.\n\n'
            + 'Ответь ровно одним словом, без точки и пояснений:\n'
            + '— same, если предмет тот же; сомневаешься — тоже same;\n'
            + '— иначе новое слово латиницей, строчными, до четырнадцати знаков, без пробелов '
            + 'и дефисов, достаточно широкое, чтобы прожить весь разговор, а не две реплики.',
        },
        ctx: {
          me: 'тема',
          roster: this.roster,
      ...
      if (!/^[a-z][a-z0-9_]{0,13}$/.test(word) || word === 'same' || word === st.topic) return;
      st.topic = word;
      this.save(room);
      this.store.listeners.forEach((fn) => {
        try { fn({ kind: 'topic', room, topic: word, ts: Date.now() }); } catch { /* ignore */ }
      });
    } catch (e) {
      this.log.error(e);
    } finally {
  ...
  handBack(room) {
    ...
    const name = here.find((n) => (this.roster[n]?.role ?? '').toLowerCase() === 'продюсер')
      ?? this.duty(room).find((n) => here.includes(n))
      ?? here[0];
    if (!name) return;
    const st = this.state(room);
    st.closed = true;
