  ...
  flush(room) {
    ...
    try {
      writeState(this.stateDir, room, {
        ...
        topic: st.topic, doing: st.doing, topicAt: st.topicAt, seen, sessions,
      });
    } catch (e) {
      this.log.error(e);
    }
  ...
  dispatch(room, msg) {
    ...
    if (!targets.length) {
      if (msg.from === this.config.user) {
        ...
        // Отвечает тот, кого задела тема, а не постоянная тройка: разговор про текст
        // и деньги, в котором отвечает Инженер, — это не разговор про текст и деньги.
        const byTopic = this.drawn(room, msg.text);
        // Тема не зацепила никого — отвечают дежурные режима «Открытый». Это и «привет»,
        // и вопрос, для которого в ролях нет слов: лучше двое общих, чем тишина.
        const duty = this.duty(room).filter((n) => this.agents.has(n));
        targets = byTopic.length ? byTopic : (duty.length ? duty : this.here(room).slice(0, 1));
        ...
        if (toMe && this.agents.has(prev.from)) targets = [prev.from];
      }
    }
      ...
      const res = await probe.speak({
        ...
        step: {
          ...
          prompt: 'На этот ход ты не участник разговора и в чат не отвечаешь. Нужно два слова.\n\n'
            ...
            + 'латиницей, строчными, до четырнадцати знаков, без пробелов и дефисов. Любое '
            + 'из двух можно оставить прежним, написав вместо него same; сомневаешься — same. '
            + 'Бери широко: слово должно прожить весь разговор, а не две реплики.',
        },
        ctx: {
          me: 'знак',
          roster: this.roster,
