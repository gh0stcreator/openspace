...
export class Orchestrator {
  constructor({ store, config, log = console, build = buildAgent }) {
    this.store = store;
    this.config = config;
    this.log = log;
  ...
  state(room) {
    if (!this.rooms.has(room)) {
      this.rooms.set(room, {
        autoTurns: 0, paused: false, jobs: [], running: false,
        ...
        mode: null, step: 0, run: 0, pending: [], upto: 0,
      });
    }
  ...
  nextStep(room) {
    ...
    if (!step) {
      ...
      this.store.append(room, {
        from: 'system',
        kind: 'system',
        text: `@${this.config.user}, круги пройдены — слово за вами`,
        mentions: [this.config.user],
      });
      return;
    }

    st.pending = stepTargets(step, this.names, this.roster);
    if (!st.pending.length) return this.nextStep(room);

    // Граница шага — тихой строкой в самой ленте: другого места, где шаг виден, нет.
    // Участникам служебные строки не уходят, у них шаг назван в промпте хода.
    this.store.append(room, {
      from: 'system',
      kind: 'system',
      text: `${st.step + 1}/${mode.steps.length} · ${step.name}${step.hear ? '' : ' · вслепую'}`,
      mentions: [],
    });
  ...
  dispatch(room, msg) {
    ...
    if (st.mode) {
      ...
      const asked = resolveTargets(msg, this.names);
      if (asked.length) {
    ...
    if (!targets.length) {
      if (msg.from === this.config.user) {
        ...
        const duty = this.duty().filter((n) => this.agents.has(n));
  ...
  duty() {
    ...
    return step ? stepTargets(step, this.names, this.roster) : [...this.names];
  }
