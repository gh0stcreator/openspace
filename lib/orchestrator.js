...
export class Orchestrator {
   ...
   */
  async touched(room, text, exclude = []) {
    ...
    const card = (n) => {
      ...
      return `- @${face(n)}: ${cares}.${motive}`;
    };
    const answer = await ask(`В разговоре прозвучало:\n«${String(text).slice(0, 1200)}»\n\n`
      + `Кому из них есть что на это сказать — по-настоящему, а не из вежливости?\n`
      + `${pool.map(card).join('\n')}\n\n`
      + `Назови одного — того, кого это задело сильнее всех. Не задело никого — ответь `
      + `«никто», и это нормальный ответ: молчать тут не запрещено. Ничего, кроме ника `
      + `или слова «никто», не пиши.`, { cwd: this.config.workdir });
    if (!answer) return this.drawn(room, text, exclude, 1);
    const said = answer.toLowerCase();
    const hit = pool.find((n) => said.includes(face(n).toLowerCase()));
    return hit ? [hit] : [];
  }
  ...
  async dispatch(room, msg) {
    ...
    if (!targets.length) {
      if (msg.from === this.config.user) {
        ...
        if (!targets.length && chatty(st) && st.autoTurns < CHATTY) {
          ...
          targets = await this.touched(room, msg.text, [msg.from, ...waiting]);
        }
      }
    }
    ...
    if (chatty(st) && targets.length) {
      ...
      this.touched(room, msg.text, skip)
        .then(([hit]) => {
          if (hit) this.enqueue(room, () => this.turn(room, hit, { after: true }));
        })
