...
import { BUILTIN, listModes, loadMode, sideOf, stepTargets } from './modes.js';
...
export class Orchestrator {
  ...
  aside(room, mode, step) {
    ...
    return {
      ...
      aside: true,
      until: 'все ответят',
      prompt: `Ты ведёшь режим «${mode.title}». Идёт шаг «${step?.name ?? ''}» — ${st.step + 1} из `
        + `${mode.steps.length}, и @${this.config.user} написал посреди него. Ответь ему одной строкой, `
  ...
  dispatch(room, msg) {
    ...
    if (st.mode) {
      ...
      const mode = loadMode(st.mode);
      const step = mode.steps[st.step];
      if (step?.until === 'человек' && !st.pending.length) return this.nextStep(room);
      ...
      st.asideOn = mark;
      this.enqueue(room, () => this.turn(room, host, { step: this.aside(room, mode, step) }));
      return;
    }
  ...
  async speakTurn(room, name, agent, ctx) {
    ...
    try {
      ...
      const posted = this.post(room, {
        ...
        text,
        files,
        // В реплику кладём подпись и знак, но не список ролей: он живёт в файле режима,
        // а в ленте повторялся бы в каждой строке.
        side: ctx.step?.side
      ...
      if (ctx.step?.aside && /^заново\b/i.test(posted.text)) this.restart(room);
      return true;
    } finally {
      agent.busy = false;
