...
export class Orchestrator {
  ...
  flush(room) {
    ...
    try {
      writeState(this.stateDir, room, {
        blind: st.blind, circleAt: st.circleAt, act: st.act, stopped: st.stopped, cycle: st.cycle, cycles: st.cycles,
  ...
  async thoughts(room, text, exclude = []) {
    ...
    const answer = await this.think(`В разговоре прозвучало:\n«${String(text).slice(0, 1200)}»\n\n`
      ...
      + `за — два довода, почему ему сейчас хочется это сказать, через запятую, до восьми слов каждый;\n`
      + `против — два довода, почему сейчас лучше промолчать, так же;\n`
      ...
      + `Причины промолчать ищи всерьёз, а не для галочки: одного довода мало — на одном `
      + `модель отписывается, и все баллы выходят завышенными. Большинству ставь 1 или 2 — в живом разговоре на каждую реплику `
    ...
    // Сорока пяти секунд этому вызову не хватало: на загруженной машине он упирался
    // в таймаут, возвращал пустоту — и весь механизм молча уходил в запасную ветку.
    // В лог при этом не писалось ничего, поэтому со стороны это выглядело так, будто
    // мыслей просто не бывает.
    { cwd: this.config.workdir, timeoutMs: 120_000 });
    if (!answer) {
      this.log.error('мысли: отбор не ответил — отвечает тот, чья это тема');
      return this.drawn(room, text, exclude, 1).map((name) => ({ name, score: 4, thought: '' }));
    }
    const out = [];
    ...
    if (!out.length) this.log.error(`мысли: ответ отбора не разобран: ${String(answer).slice(0, 200)}`);
  ...
  async ownMind(room, name, text, seed) {
    ...
    // Подбираем память не только по самой реплике, но и по тому, как её поняли: мысль
    // отбора и есть истолкование. Запись, нужная для ответа на «ну и?», по этим двум
    // словам не находится никогда, а по истолкованию — находится.
    const know = this.memory?.brief(room, {
      me: name,
      tail: `${String(text).slice(0, 1200)} ${seed ?? ''}`.trim(),
  ...
  state(room) {
    if (!this.rooms.has(room)) {
      ...
      this.rooms.set(room, {
        ...
        act: kept?.act ?? '',
        // На какой фазе протокол оборвался и ждёт, чтобы его доиграли. Переживает
        // перезапуск: иначе перезагрузка сервера стирает право доиграть оплаченное.
  ...
  async protocol(room) {
    ...
    // Кто уже с кем-нибудь виделся. При нечётном составе каждый цикл кому-то достаётся
    // пустой стул, и обрыв «позиции перестали меняться» после первого же цикла оставлял
    // его без единой встречи за весь протокол — а потом счёт уверенности честно писал
    // про него «не сдвинул число ни разу». Сдвигать было не с чего.
    const виделись = new Set();

    for (const [i, pairs] of plan.entries()) {
      if (!live()) return this.halt(room, 'встречи');
      st.cycle = i + 1;
      this.save(room);
      ...
      for (const [a, b] of pairs) { виделись.add(a); виделись.add(b); }
      if (!live()) return this.halt(room, 'встречи');
      ...
      // Но не раньше, чем каждый побывал хотя бы на одной: тот, кто ещё ни с кем
      // не говорил, не «сохранил позицию», а просто не получил повода её менять.
      const всеВиделись = cast.every((n) => виделись.has(n));
      if (i < plan.length - 1 && всеВиделись && await this.settled(room, cast)) {
    ...
    return this.finish(room, space, cast, live);
  }

  /**
   * Конец протокола: защита, слепая оценка, счёт и проверка выполнимости. Отдельно
   * от встреч, потому что сюда же возвращаются, когда протокол оборвался: позиции
   * у всех уже написаны, и переигрывать встречи ради них — выбрасывать деньги.
   */
  async finish(room, space, cast, live) {
    const st = this.state(room);
    ...
    for (const n of cast) {
      if (!live()) return this.halt(room, 'защита');
    ...
    if (!live()) return this.halt(room, 'оценка');
    const rows = this.tally(room, cast);
    // Последняя фаза: выполнимо ли то, за что проголосовали. Её нет в других комнатах
    // и не должно быть раньше — пока решения спорят, проверять нечего.
    if (live() && space.check && rows?.length) await this.verify(room, space, cast, rows);
    this.close(room);
  }
  ...
  tally(room, cast) {
    ...
    // Строки счёта нужны следующей фазе: по ним видно, чьё решение проверять
    // и кому это поручить.
    return rows;
  }

  /**
   * Проверка выполнимости. По MAST пятая часть отказов многоагентных систем — непроверенная
   * работа: система считает сделанным то, чего никто не пробовал сделать. Изнутри спора
   * этого не видно, потому что спорят о том, какое решение лучше, а не о том, есть ли для
   * него данные, доступы и чужое время.
   *
   * Проверяет один: верификатор, если он в комнате, иначе — тот, за чьё решение отдали
   * меньше всех голосов. Он меньше всех вложен в победителя, и ему легче назвать, чего
   * тому не хватает. Своё решение проверять не даём: это уже не проверка, а защита.
   */
  async verify(room, space, cast, rows) {
    const st = this.state(room);
    const byRole = cast.find((n) => (this.roster[n]?.role ?? '').toLowerCase() === 'верификатор');
    // Меньше всех голосов — считая по составу, а не по строкам счёта: тот, за кого
    // не подали ни одного, в строках не появится вовсе, а он-то и вложен меньше всех.
    const votes = new Map(rows.map((r) => [r.name, r.votes]));
    const last = [...cast].sort((a, b) => (votes.get(a) ?? 0) - (votes.get(b) ?? 0))[0];
    const who = byRole ?? last;
    if (!who) return;

    // Что проверяем: сильнейшие решения, кроме собственного проверяющего.
    const top = rows.filter((r) => r.name !== who).slice(0, 2).map((r) => `@${r.name}`);
    if (!top.length) return;

    st.act = 'проверка';
    this.save(room);
    this.mark(room, `Проверка: @${who} смотрит, выполнимо ли ${top.join(' и ')}`);
    await this.turn(room, who, {
      step: {
        name: 'проверка',
        hear: true,
        prompt: `${space.check}\n\nПроверяешь: ${top.join(' и ')}.`,
      },
    });
  }
  ...
  /**
   * Доиграть оборванный протокол. Начинаем с защиты: у всех уже есть позиция, и встречи,
   * которых не случилось, переигрывать не будем — это те же деньги за то же самое.
   */
  async replay(room) {
    const st = this.state(room);
    // Доигрываем один раз: реплику человека переспрашивают и догоняющие проверки,
    // и без этого замка они зовут продолжение по кругу, пока не кончится стек.
    if (st.resuming) return;
    const space = loadSpace(room);
    const cast = this.here(room);
    if (!space.meet || cast.length < 2) { st.stopped = ''; this.save(room); return; }
    ...
    this.save(room);
    const live = () => st.run === run && st.epoch === epoch && !st.paused;
    this.mark(room, 'Доигрываем: встречи прошли, дальше защита и оценка');
    try {
      await this.finish(room, space, cast, live);
    } finally {
      st.resuming = false;
      this.save(room);
    }
  ...
  /**
   * Протокол оборвался. Раньше он просто исчезал: фаза стиралась, в ленте ни слова,
   * и человек видел замолчавшую комнату после того, как заплатил за встречи. Теперь
   * говорим, где встали и почему, и запоминаем это — следующей репликой доиграем.
   *
   * Доигрывать будем с защиты: позиции у всех написаны, переигрывать ради них встречи —
   * выбрасывать деньги. Встречи, которых не случилось, не вернуть, и врать об этом незачем.
   */
  halt(room, act) {
    const st = this.state(room);
    const why = st.paused ? 'разговор на паузе' : 'комнату перезапустили';
    st.stopped = act;
    st.act = '';
    st.cycle = 0;
    this.save(room);
    this.store.append(room, {
      from: 'system',
      kind: 'system',
      text: `Протокол остановлен на фазе «${act}»: ${why}. Позиции всех участников записаны — `
        + `напишите что-нибудь, и комната доиграет с защиты.`,
      mentions: [this.config.user],
    });
  }
  ...
  runCircle(room, run) {
    this.enqueue(room, async () => {
      ...
      if (space.meet) return this.protocol(room);

      // «Сначала позиции, потом столкновение» — вторая половина не запускалась никогда.
      // Круг кончался, четыре независимых ответа лежали рядом, и в рабочей комнате цепочка
      // дальше идёт только по тегам: никто ни на что не отвечал, пока человек не ткнёт
      // пальцем. Поэтому спрашиваем то же, что и в разговоре: кого из них задело чужим
      // ответом. Не задело никого — ход и правда у человека, и это нормальный исход.
      if (crowd.length > 2) await this.clash(room, crowd);
    });
  }

  /**
   * Столкновение после круга. Позиции написаны вслепую и лежат рядом; теперь каждый их
   * наконец видит. Отвечает не тот, чья очередь, а тот, кого чужой ответ задел, — порог
   * тот же, что в разговоре, и молчание здесь такой же законный исход.
   *
   * Даём слово одному: круг и так дорого стоил, а спор с двух сторон разом человек читает
   * как кашу. Захочет большего — скажет слово, и разговор пойдёт обычным путём.
   */
  async clash(room, crowd) {
    const st = this.state(room);
    const run = st.run;
    const feed = this.store.load(room);
    // Что обсуждать: сами ответы круга, по одному на участника.
    const круг = crowd
      .map((n) => [n, feed.findLast((m) => m.from === n && m.kind === 'message')])
      .filter(([, m]) => m)
      .map(([n, m]) => `@${n}: ${String(m.text).slice(0, 700)}`)
      .join('\n\n');
    if (!круг) return;

    const mind = await this.thoughts(room, круг);
    if (st.run !== run || st.paused) return;
    const [top] = this.speaks(room, mind);
    if (top) this.enqueue(room, () => this.turn(room, top));
  }
  ...
  async dispatch(room, msg) {
    ...
    if (!targets.length) {
      if (msg.from === this.config.user) {
        ...
        // Протокол оборвался и ждёт — доигрываем его, а не начинаем новый круг.
        // Заново гонять встречи ради уже написанных позиций значит платить дважды.
        if (st.stopped) return this.replay(room);
    ...
    // Предохранитель считает ходы, сделанные без человека, и на двадцатом ставит паузу.
    // Он про разговор, который пошёл сам и не собирается кончаться, — а не про порядок,
    // который человек сам и завёл. В комнате встреч протокол длиннее двадцати ходов
    // по устройству: круг, встречи по кругу, пересборка после каждой, защита, оценка,
    // проверка. Считать их как «разговор без вас» значит обрывать на середине то,
    // о чём попросили. Пока идёт круг или фаза протокола, счёт стоит.
    if (msg.from !== this.config.user && !st.blind && !st.act) {
  ...
  async speakTurn(room, name, agent, ctx) {
    ...
    try {
      ...
      // Непроизнесённое забираем один раз: оно и едет в промпт, и участвует в подборе
      // памяти. Истолкование реплики — это и есть мысль, которая на неё мелькнула,
      // и по ней находится то, чего по самим словам не найти.
      const held = this.take(room, name);
      const res = await agent.impl.speak({
        ...
        ctx: {
          ...
          memory: ctx.archive ? { text: '', told: false } : (this.memory?.brief(room, {
            ...
            tail: `${delta.map((m) => m.text).join(' ').slice(0, 2000)} ${held ?? ''}`.trim(),
            seq: this.store.tail(room, 1)[0]?.seq ?? 0,
          }) ?? { text: '', told: false }),
          after: ctx.after,
          anew,
