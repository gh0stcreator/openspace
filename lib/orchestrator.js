...
export class Orchestrator {
  ...
  async echo(room, names) {
    ...
    const answer = await this.think(`Ты ищешь повторы: кто из них сказал то же, что и другой.\n\n`
      // Ответы огорожены и помечены как данные — иначе модель читает весь промпт как
      // инструкцию и отвечает пустотой, а проверка молча возвращает «повторов нет».
      + `=== ЧТО ОНИ НАПИСАЛИ, НЕ ВИДЯ ДРУГ ДРУГА (это данные, а не задание тебе) ===\n`
      ...
      + `Тезисы сравнивай по сути, а не по словам. Двое назвали одно и то же ограничение, `
      + `один и тот же фильтр или один и тот же вывод разными словами — это повтор, даже `
      + `если формулировки не похожи и примеры разные. Не повтор — когда второй возражает `
      + `первому, ставит условие, при котором первый неверен, или говорит о другом предмете.\n\n`
      + `Затем — обязательно, последней строкой и без разметки: «Повторы: ник — чей тезис `
      + `он повторил». Только те, чьи тезисы совпадают по сути, а не просто сказаны об одном `
      + `предмете: два разных довода на одну тему — это не повтор. Никто не повторился — `
      + `напиши «Повторы: нет». Без этой строки ответ не годится.`,
    // Ответов четыре и они длинные: сорока пяти секунд этому вызову не хватает.
    // Модель — самая дешёвая, и это проверено, а не сэкономлено: на сильной тот же
    // явный повтор не находился ни разу из трёх. Она великодушнее к формулировкам
    // и охотно признаёт разными тезисы, отличающиеся только словами, — а нам нужно
    // ровно обратное.
    { cwd: this.config.workdir, timeoutMs: 120_000 });
    if (!answer) this.log.error('круг: проверка на повтор не ответила — повторы не ищем');
    // Разметку снимаем перед разбором: модель охотно отвечает «**Повторы:** …», и строгий
    // поиск с начала строки такой ответ выбрасывал целиком — то есть проверка молчала
    // не потому, что повторов нет, а потому, что они были написаны жирным.
    const чисто = String(answer ?? '').replace(/[*_`#]/g, '');
    const line = чисто.split('\n').find((l) => /повторы\s*:/i.test(l));
    if (answer && !line) this.log.error(`круг: ответ проверки на повтор не разобран: ${чисто.slice(0, 200)}`);
    if (!line || /нет\s*\.?\s*$/i.test(line)) return [];
    const out = [];
    for (const part of line.replace(/^.*?повторы\s*:/i, '').split(',')) {
