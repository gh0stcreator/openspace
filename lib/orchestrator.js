...
export class Orchestrator {
  ...
  flush(room) {
    ...
    try {
      writeState(this.stateDir, room, {
        ...
        off: st.off, offBefore: st.offBefore, archiveUpto: st.archiveUpto, topic: st.topic, seen, sessions,
      });
    } catch (e) {
      this.log.error(e);
    }
  ...
  post(room, { from, text, kind = 'message', meta, files, replyTo }) {
    ...
    if (from === this.config.user) {
      ...
      // Метку темы проверяем на его реплике: участники развивают начатое, а сворачивает
      // разговор в сторону обычно он. Мимо очереди и без ожидания — ход от этого не ждёт.
      if (kind === 'message') this.retopic(room).catch(() => {});
    }
      ...
      const res = await probe.speak({
        ...
        step: {
          ...
          prompt: 'На этот ход ты не участник разговора и в чат не отвечаешь. Нужно одно слово.\n\n'
            ...
            + '— same, если про то же самое; сомневаешься — тоже same;\n'
            + '— иначе новое слово латиницей, строчными, до четырнадцати знаков, без пробелов '
            + 'и дефисов. Бери широко: memory, ui, pricing, onboarding — так, чтобы метка '
            + 'прожила весь разговор, а не две реплики.',
        },
        ctx: {
          me: 'тема',
          roster: this.roster,
