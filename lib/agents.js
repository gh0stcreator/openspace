...
class ClaudeAgent {
  ...
  async speak({ room, delta, history, step, ctx, archive = false }) {
    // Свёртка — разовый вызов чужой шапкой, не продолжение сессии носителя: --resume
    // вложил бы ей в историю чужие реплики (в том числе те, что барьер слепого шага ещё
    // не раскрыл) и навсегда осел бы токенами, которые носитель платит на каждом следующем ходу.
    const sid = archive ? null : this.sessions.get(room);
    const args = [
      '-p',
      '--output-format', 'json',
    ...
    if (!archive && parsed.session_id) this.sessions.set(room, parsed.session_id);
    if (parsed.is_error) {
...
class CodexAgent {
  ...
  async speak({ room, delta, history, step, ctx, archive = false }) {
    ...
    // Свёртка — разовый вызов, не продолжение сессии носителя (см. ClaudeAgent.speak).
    const sid = archive ? null : this.sessions.get(room);
    ...
    if (!archive && parsed.threadId) this.sessions.set(room, parsed.threadId);
