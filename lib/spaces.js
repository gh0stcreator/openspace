...
function parse(name, raw) {
  ...
  return {
    ...
    // Комната со встречами: после круга участники сходятся парами, и каждая пара говорит
    // наедине. Непустое поле «встречи» и включает этот порядок — остальные три поля
    // описывают, что делают на встрече, после неё и в конце.
    meet: String(meta['встречи'] ?? '').trim(),
    rework: String(meta['пересборка'] ?? '').trim(),
    defend: String(meta['защита'] ?? '').trim(),
    vote: String(meta['оценка'] ?? '').trim(),
    // Проверка: выполнимо ли то, за что проголосовали. Последняя фаза и единственная,
    // где не придумывают и не защищают. По MAST непроверенная работа — пятая часть
    // всех отказов многоагентных систем, и это ровно та, которую не видно изнутри спора.
    check: String(meta['проверка'] ?? '').trim(),
...
export function loadSpace(name) {
  ...
  try {
    ...
    // Белая тоже файл. Чего в файле нет, берём из умолчаний, чтобы пустая строка
    // не стёрла название.
...
export function listSpaces() {
  ...
  return fs.readdirSync(DIR)
    ...
    // Встроенная комната первой, комнаты с персонами — последними: они не про работу,
    // а про то, кем в них выходят. Отсутствие регламента особой комнату не делает:
    // разговор без задачи — такая же комната, как остальные, просто белая.
    .sort((a, b) => {
      if (a.name === BUILTIN || b.name === BUILTIN) return a.name === BUILTIN ? -1 : 1;
      const маска = (s) => (s.sides ?? []).some((x) => x.color);
      if (маска(a) !== маска(b)) return маска(a) ? 1 : -1;
...
/**
 * Расписание встреч: кто с кем и в каком цикле. Классический круговой турнир — один
 * стоит на месте, остальные поворачиваются вокруг него, — и получается, что каждый
 * встречается с каждым ровно один раз, а в каждом цикле все заняты.
 *
 * Для четверых это три цикла по две пары, для шестерых — пять по три. Цикл и есть
 * итерация мышления: между встречами каждый переписывает своё в одиночку.
 */
export function meetings(names) {
  const ring = [...names];
  if (ring.length < 2) return [];
  // Нечётному составу нужен пустой стул: тот, кому он достался, в этом цикле отдыхает.
  if (ring.length % 2) ring.push(null);
  const half = ring.length / 2;
  const rest = ring.slice(1);
  const plan = [];
  for (let i = 0; i < ring.length - 1; i += 1) {
    const round = [ring[0], ...rest];
    const pairs = [];
    for (let j = 0; j < half; j += 1) {
      const a = round[j];
      const b = round[round.length - 1 - j];
...
export function saveSpace(space) {
  ...
  const head = [
    ...
    `встречи: ${space.meet ?? ''}`,
    `пересборка: ${space.rework ?? ''}`,
    `защита: ${space.defend ?? ''}`,
    `оценка: ${space.vote ?? ''}`,
    `проверка: ${space.check ?? ''}`,
...
/** Удалить комнату. Белую не трогаем: без неё некуда возвращаться. */
