...
export function loadSpace(name) {
  ...
  // Комнаты без файла — старые ленты: они открываются и работают как общая.
  ...
  try {
    ...
    // Общая тоже файл. Чего в файле нет, берём из умолчаний, чтобы пустая строка
    // не стёрла название.
...
export function listSpaces() {
  ...
  return fs.readdirSync(DIR)
    ...
    // Общая первой, комнаты без регламента — последними: они не про работу,
...
/** Удалить комнату. Общую не трогаем: без неё некуда возвращаться. */
