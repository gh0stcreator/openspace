...
export const DEFAULTS = {
  ...
  maxAutoTurns: 12,
  // Потолок стоимости одного вопроса в комнате встреч. Предохранитель на ходы там
  // выключен нарочно — иначе протокол рвался бы на середине, — а сам протокол на четверых
  // это под полсотни полных ходов. Считаем в токенах, потому что ход ходу рознь: пересборка
  // стоит впятеро меньше защиты. Ноль снимает потолок совсем.
  protocolTokens: 1_500_000,
